    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.id">
          <div class="pro-check">
            <el-checkbox :value="check(item.status)" @change="checkChange($event,index)"></el-checkbox>
          </div>
          <div class="pro-img">
...
export default {
  ...
  methods: {
    ...
    handleChange(currentValue, key, productId) {
      // 当修改数量时，默认勾选
      this.updateShoppingCart({ key: key, prop: "status", val: 1 });
      // 向后端发起更新购物车的数据库信息请求
      this.$axios
        .get("http://47.100.75.161:8081/cart/updateCart", {
          params:{
