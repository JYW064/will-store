    ...
    <div class="block">
      <el-carousel height="460px">
        <el-carousel-item v-for="item in carousel" :key="item.carouselId">
          <img style="height:460px;" :src="$target + item.img" :alt="item.describes" />
        </el-carousel-item>
      </el-carousel>
    </div>
    <!-- 轮播图END -->
...
export default {
  ...
  created() {
    // 获取轮播图数据
    this.$axios
        .post("http://47.100.75.161:8081/index/getCarousel", {})
        .then(res => {
    ...
    this.getHot(
        [2, 3, 4],
        "applianceList",
        "http://47.100.75.161:8081/index/getHotProducts"
    );
    this.getHot(
        [5 ,6 , 7, 8],
        "accessoryList",
  ...
  methods: {
    ...
    getPromo(categoryId, val, api) {
      api = api != undefined ? api : "http://47.100.75.161:8081/index/getPromoProducts";
      this.$axios
          .post(api, {
            categoryId
          })
    ...
    getHot(categoryId, val, api) {
      api = api != undefined ? api : "http://http://47.100.75.161:8081/index/getHotProducts";
      this.$axios
          .post(api, categoryId
          )
          .then(res => {
