...
export default {
  ...
  watch: {
    ...
    getUser: function(val) {
      if (val === "") {
        ...
        this.$axios
          .post(
              "http://47.100.75.161:8081/cart/getCart"
              //"http://127.0.0.1:8080/cart/getCart"
              , {
            userId: val.userId
          })
          .then(res => {
            if (res.data.code === "001") {
              // 001 为成功, 更新vuex购物车状态
              this.setShoppingCart(res.data.result);
            } else {
              // 提示失败信息
  ...
  methods: {
    ...mapActions(["setUser", "setShowLogin", "setShoppingCart","setToken"]),
    login() {
